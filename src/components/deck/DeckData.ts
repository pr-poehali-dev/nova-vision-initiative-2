export const packages = [
  {
    level: "Генеральный партнёр",
    price: "4 000 000 ₽+",
    color: "from-gold to-terracotta",
    textColor: "text-gold",
    perks: [
      "Логотип на афишах и баннерах",
      "Упоминание на мероприятии",
      "Расширенные публикации в соцсетях",
      "Упоминание в пресс-релизах и СМИ",
      "Фото и видео с мероприятия",
      "Эксклюзивное брендирование сцены",
      "Фуршет",
      "Именная номинация с брендом",
      "* Обсуждение индивидуальных пожеланий",
    ],
  },
  {
    level: "Титульный партнёр",
    price: "4 000 000 ₽",
    color: "from-rose to-indigo",
    textColor: "text-rose",
    perks: [
      "Логотип на афишах и баннерах",
      "Упоминание на мероприятии",
      "Расширенные публикации в соцсетях",
      "Упоминание в пресс-релизах и СМИ",
      "Фото и видео с мероприятия",
      "Эксклюзивное брендирование сцены",
      "Фуршет",
    ],
  },
  {
    level: "Официальный партнёр",
    price: "3 000 000 ₽",
    color: "from-sage to-indigo",
    textColor: "text-sage",
    perks: [
      "Логотип на афишах и баннерах",
      "Упоминание на мероприятии",
      "Публикации в соцсетях",
      "Упоминание в пресс-релизах",
      "Фото и видео с мероприятия",
      "Брендирование зоны на площадке",
      "Фуршет",
    ],
  },
  {
    level: "Информационный партнёр",
    price: "2 000 000 ₽",
    color: "from-stone to-stone/50",
    textColor: "text-muted-foreground",
    perks: [
      "Логотип на афишах и баннерах",
      "Упоминание на мероприятии",
      "Публикации в соцсетях",
      "Упоминание в пресс-релизах",
      "Фуршет",
    ],
  },
]

export const timeline = [
  { date: "Март 2026", title: "Старт партнёрской кампании", desc: "Финальные переговоры со спонсорами, подписание договоров" },
  { date: "Апрель 2026", title: "Кастинг и отбор участниц", desc: "Публичный кастинг, старт PR-активностей, анонсы спонсоров" },
  { date: "Май 2026", title: "Медиакампания", desc: "Посты в соцсетях, билборды, упоминания в СМИ" },
  { date: "Июнь 2026", title: "Финальная подготовка", desc: "Репетиции, брендинг площадки, монтаж стендов спонсоров" },
  { date: "19 сентября 2026", title: "День шоу", desc: "Гала-вечер, 5 000+ гостей, звёздный артист на сцене" },
  { date: "Октябрь–Ноябрь", title: "Постпродакшн", desc: "Видеоотчёт, пресс-релизы, медиааналитика для спонсоров" },
]

export const SLIDES_COUNT = 12

export const SLIDE_TITLES = [
  "О событии", "Аудитория", "Ценность", "Медиаплан", "Пакеты",
  "Активации", "Кейсы", "Таймлайн", "Артист", "ROI", "Контакты", "CTA"
]